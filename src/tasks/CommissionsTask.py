...
class CommissionsTask(BaseDNATask):
    ...
    def use_skill(self, skill_time):
        ...
        # 检查是否启用多次技能释放
        if self.config.get("启用多次技能释放", False):
            return self.use_multiple_skills(skill_time)
        else:
            return self.use_single_skill(skill_time)

    def use_single_skill(self, skill_time):
        """传统的单次技能释放"""
        if time.time() - skill_time >= self.config.get("技能释放频率", 5):
    ...
    def use_multiple_skills(self, skill_time):
        """多次技能释放机制"""
        current_time = time.time()
        skill_frequency = self.config.get("多次释放冷却时间", 8.0)
        ...
        # 获取配置参数
        burst_count = self.config.get("多次释放次数", 3)
        burst_interval = self.config.get("多次释放间隔", 0.2)
        skill_type = self.config.get("使用技能", "战技")

        # 使用通用方法执行多次技能释放
        skill_time = current_time  # 更新技能时间
        self.burst_skill_release(skill_type, burst_count, burst_interval)
